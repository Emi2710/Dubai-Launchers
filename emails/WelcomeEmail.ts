export function generateWelcomeEmail(first_name: string, passwordLink: string) {
  return `
  <!DOCTYPE html>
  <html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Bienvenue sur Dubai Launchers!</title>
  </head>
  <body style="margin:0; padding:0; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif;">
    
    <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color:#1a1b3a; width:100%; min-height:100vh;">
      <tr>
        <td align="center">

          <table width="600" cellpadding="0" cellspacing="0" border="0" style="background-color:#1a1b3a; color:white;">
            <tr>
              <td style="padding:20px 40px; text-align:center;">

                <h1 style="font-size:32px; font-weight:bold; margin:30px 0 20px; line-height:1.2; color:white;">
                  Bienvenue sur <span style="color:#a855f7;">Dubai Launchers</span>
                </h1>

                <p style="font-size:18px; color:white; margin-bottom:30px; line-height:1.5;">
                  Nous sommes ravis de vous compter parmi nous.
                </p>

                <p style="font-size:16px; color:white; margin-bottom:40px; line-height:1.6;">
                  Pour accéder à votre espace Dubai Launchers, veuillez d'abord créer votre mot de passe en cliquant sur le bouton ci-dessous.
                </p>

                <a href="${passwordLink}" style="display:inline-block; background:linear-gradient(135deg, #8b5cf6 0%, #a855f7 100%); color:white; text-decoration:none; padding:16px 32px; border-radius:12px; font-weight:600; font-size:16px; margin:20px 0; box-shadow:0 4px 15px rgba(168,85,247,0.3);">
                  Créer votre mot de passe <span style="margin-left:8px;">→</span>
                </a>

                <p style="font-size:14px; color:#94a3b8; margin-top:30px;">
                  Ce lien va expirer dans 24h.
                </p>

              </td>
            </tr>

            <tr>
              <td style="padding:40px 20px 20px; text-align:center; border-top:1px solid rgba(255,255,255,0.1); margin-top:40px;">
                <p style="font-size:14px; color:#94a3b8; margin-bottom:10px;">
                  © 2025 Dubai Launchers. All rights reserved.<br>
                  Dubai, United Arab Emirates
                </p>
              </td>
            </tr>
          </table>

        </td>
      </tr>
    </table>

  </body>
  </html>`;
}
